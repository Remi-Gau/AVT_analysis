...
ParOrNonPar = 2; %input(message);
%Parameter grid defining parameter space through which to perform a
%brute-force search for values to be used as initial guesses in iterative
%parameter search.
searchGrid.alpha = 0:.01:1;
searchGrid.beta = logspace(1,3,250);
...
figure('name', 'PsychMetric functions - A', ...
    'position', FigDim)
...
for SubjInd = 1:length(Subjects)
    ...
    cd(fullfile(StartDirectory,'Subjects_Data', strcat('Subject_',  num2str(Subjects(SubjInd))), 'PsyPhy'))
    ...
    % Psychometric curve
    LevelsUsedFineGrain=linspace(min(Loc),max(Loc),100);
...
print(gcf, 'PsychometricCurve_A.tif', '-dtiff')
