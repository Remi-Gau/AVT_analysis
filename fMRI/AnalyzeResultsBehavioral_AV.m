...
for SubjInd = 1:length(Subjects)
    ...
    cd(fullfile(StartDirectory, strcat('Subject_', num2str(Subjects(SubjInd))), 'Archives'))
    ...
    for iFile = 1:length(dir(strcat('Logfile_Subject_', num2str(Subjects(SubjInd)), '_Run_100*.txt')))
        ...
        if Subjects(SubjInd)==1 || (Subjects(SubjInd)==4 && iFile>2)
            ComSrcResp(ComSrcResp==3)=1;
            ComSrcResp(ComSrcResp==4)=0;
        else
            ComSrcResp(ComSrcResp==1)=1;
            ComSrcResp(ComSrcResp==2)=0;
        end
...
%         sum(IncDiff)
    ...
    %%
    figure('name', ['Subject ' num2str(Subjects(SubjInd))], 'position', [100 100 600 600]+300)
    hold on
    grid on
    plot(AccuracyCon, '-+b')
    plot(AccuracyInc, '-+r')
    ...
    set(gca, 'xtick', 1:3, 'xticklabel', [' 50';'100';'200'])
    xlabel('Duration (ms)')
