function  fig_h = plot_PCM_ind(M, G, G_hat, T_ind, D, T_ind_cross, theta_ind, theta_ind_cross, G_pred_ind, G_pred_ind_CV, RDMs, RDMs_CV, opt);
...
for iSub = 1:NbSub
    ...
    for i=1:3
        ...
        MIN = min(T.likelihood_norm(2:end-1));
        MAX = max(T.likelihood_norm(2:end-1));
...
fig_h(end+1)=figure('name', opt.FigName,'Position', opt.FigDim , 'Color', [1 1 1]);
...
for iM=2:numel(M)-1
    ...
    if iM==2
        ...
        fig_h(end+1)=figure('name', ['RDM_{PCM}-'  opt.FigName] ,'Position', opt.FigDim , 'Color', [1 1 1]);
        ...
        fig_h(end+1)=figure('name', ['RDM_{RSA}-'  opt.FigName] ,'Position', opt.FigDim , 'Color', [1 1 1]);
    ...
    subplot(NbSub,7,repmat([1:3],NbSub,1) + repmat(7*(0:NbSub-1)',1,3))
    hold on
    ...
    for iSub=1:NbSub
        ...
        %         RDM = rsa.util.rankTransform_equalsStayEqual(RDM,1);
        %         imagesc(RDM)
        imagesc(G_pred_ind_CV{end}(:,:,iSub))
        iSubplot = SetAxis(iSubplot,ColorMap);
        if iSub==1
            %             t=title('RDM_{pred}');
            t=title('G_{free CV}');
