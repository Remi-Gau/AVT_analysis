...
for SubjInd = 1:length(Subjects)
    ...
    FigDir = fullfile(pwd, 'Training');
    Opt.FigDir = fullfile(pwd, 'Training');

    copyfile('Logfile_*Run_31*.txt', 'Training')
    copyfile('Trial_List_Subject_*Run_31*.txt', 'Training')
    copyfile('Side_List_Subject_*Run_31*.txt', 'Training')
    copyfile('GazeData_Subject_*Run_31*.txt', 'Training')
    
    cd(fullfile(StartDirectory, 'Subjects_Data', strcat('Subject_', num2str(Subjects(SubjInd))), 'Training'))
  
    LogFileList = dir(strcat('GazeData_Subject_', num2str(Subjects(SubjInd)), '_Run_31*.txt'));
    
    for iFile = length(LogFileList)
        ...
        if Opt.Print
            print(gcf, fullfile(FigDir, ['OverviewTrials_' EyeData.Name '_Run_' num2str(iFile) '.tif']), '-dtiff')
        end
        ...
        EyeData.Pos_x = EyeData.Pos_x(TrialsDataSamples)-mean(EyeData.Pos_x(TrialsDataSamples));
        EyeData.Pos_y = EyeData.Pos_y(TrialsDataSamples)-mean(EyeData.Pos_y(TrialsDataSamples));
        ...
        tmp = find(EyeData.Blink==1);
        tmp(tmp<3)=[];
        EyeData.Blink(tmp+1) = 1;
        EyeData.Blink(tmp-1) = 1;
        EyeData.Blink(tmp+2) = 1;
        EyeData.Blink(tmp-2) = 1;  
        ...
        X = EyeData.Pos_x;
        X(EyeData.Blink==1)=NaN;
        stdX = mad(X,1);
        devX = abs(X - median(X));
        
        Y = EyeData.Pos_y;
        Y(EyeData.Blink==1)=NaN;
        stdY = mad(Y,1);
        devY = abs(Y - median(Y));
...
%         Remove = [];
        ...
        for iMarker = 1:numel(TrialsStarts)
            ...
            if iTrialEnd-iTrialStart-NbBlkPoint<(Opt.DataPtsPerTrial*Opt.PropDataPtsPerTrial) || ...
                    ...
                    numel(ValidTimePoints(ValidTimePoints==1))~=numel(EyeData.Blink(iTrialStart:iTrialEnd)==-1)
                ...
                fprintf(' Trial %i: not enough data.\n', iMarker )
...
%                 ValidTimePoints = CollectedTimes;
        ...
        if Opt.Print
            print(gcf, fullfile(FigDir, ['OverviewCleanedTrials_' EyeData.Name '_Run_' num2str(iFile) '.tif']), '-dtiff')
        end
        ...
        if Opt.Print
            print(gcf, fullfile(FigDir, ['Feedback_Trials_' EyeData.Name '_Run_' num2str(iFile) '.tif']), '-dtiff')
        end
