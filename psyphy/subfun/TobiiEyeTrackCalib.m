...
if isempty(CalibStart)
    CalibStart = find(Marker>0, 1, 'first');
end
...
if(mod(Marker(TrialsStarts(1)),2)==0)
    TrialsStarts(1)=[];
end
if numel(TrialsStarts)/2<NbCalibPts
    FirstCalibPoint=Marker(TrialsStarts(1));
else
    FirstCalibPoint=1;
end
...
MaxCalibSamples = round(CalibtionDur/(1000/SamplingFreq));
PercentMissingDataCalib = ...
round ( ...
( MaxCalibSamples - (MaxCalibSamples - numel(CalibDataSamples)) - sum(isnan(EyeData.Pos_x(CalibStart:CalibEnd))))   ...
    / MaxCalibSamples * 100 ...
    );
...
for iTrial = 1:floor(numel(TrialsStarts)/2)
    ...
    if Time(IndexTimeEnd)-Time(IndexTimeStart)<CalibDur-1000 || ...
        ...
        fprintf(' Trial %i: Calibration time is off.\n', iTrial )
        ...
        if numel(Samples)>SamplingFreq/2 %We make sure that we have at least more than half a second worth of data
            ...
            % hight reliability).
            if mad(X,1)<PPD*2 && mad(Y,1)<PPD*2

                % We store the median X and Y value of that
                % fixation trial.
                X_CalibrationGrid(FirstCalibPoint+iTrial-1)=median(X);
                Y_CalibrationGrid(FirstCalibPoint+iTrial-1)=median(Y);

                % We plot this trial as well as its median value
                % and mean absolute error to the median
                plot(X, Y, Color(FirstCalibPoint+iTrial-1))
                errorbar(median(X), median(Y), mad(Y,1), 'or')
                herrorbar(median(X), median(Y), mad(X,1), 'or')
                
            else
                fprintf(' Trial %i: data is too variable.\n', iTrial )

            end
            
        else
            fprintf(' Trial %i: not enough data points left.\n', iTrial )
...
EyeData.PixPerDegX = mean(diff(X_CalibrationGrid))/mean(diff(AzimuthLvls));
