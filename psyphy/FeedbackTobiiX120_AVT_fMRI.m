...
for SubjInd = 1:length(Subjects)
    ...
    FigDir = fullfile(pwd, 'Behavioral');
    Opt.FigDir = fullfile(pwd, 'Behavioral');

    copyfile('Logfile_*Run_31*.txt', 'Behavioral')
    copyfile('Trial_List_Subject_*Run_30*.txt', 'Behavioral')
    copyfile('Side_List_Subject_*Run_30*.txt', 'Behavioral')
    copyfile('GazeData_Subject_*Run_30*.txt', 'Behavioral')
    
    cd(fullfile(StartDirectory, 'Subjects_Data', strcat('Subject_', num2str(Subjects(SubjInd))), 'Behavioral'))
  
    LogFileList = dir(strcat('GazeData_Subject_', num2str(Subjects(SubjInd)), '_Run_30*.txt'));
...
%         Remove = [];
        ...
        t= text(.7,sum(ToPlot(1,:))+1,sprintf('Valid trials : %i/%i', ...
