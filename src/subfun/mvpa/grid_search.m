function [args, grid] = grid_search(trdata, trlabel, nfold, opt)
    %
    ...
    if numel(grid.first.param) == 1
        ...
        for i = 1:length(grid.first.acc)
            ...
            if opt.svm.kernel
                ...
                acc = get_accuracy(trdata, trlabel, nfold, sessions_list, args);
        ...
        grid.second.param = ...
          grid.second.param(1):(grid.second.param(2) - ...
                                grid.second.param(1)) / nsteps:grid.second.param(2);
...
function acc = get_accuracy(trdata, trlabel, sessions, sessions_list, args)
