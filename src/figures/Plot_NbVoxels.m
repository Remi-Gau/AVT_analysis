...
for iToPlot = 1:size(ToPLot, 1)
    ...
    for iROI = 1:numel(ToPLot{iToPlot, 1})
        ...
        for iSubj = 1:NbSub
            plot((iROI - 1) * 1 + Scatter(iSubj), AllSubjects_Data(Idx).size.data(iSubj, 1), ...
                 'marker', '.', ...
                 'markersize', 30, ....
        ...
        for iSubj = 1:NbSub
            plot((iROI - 1) * 1 + Scatter(iSubj), AllSubjects_Data(Idx2).size.data(iSubj, 1), ...
                 'marker', '.', ...
                 'markersize', 30, ....
                 'color', COLOR_Subject(iSubj, :));
        end
...
for iToPlot = 1:size(ToPLot, 1)
    ...
    for iROI = 1:numel(ToPLot{iToPlot, 1})
        ...
        for iSubj = 1:NbSub
            plot((iROI - 1) * 1 + Scatter(iSubj), AllSubjects_Data(Idx).size.data(iSubj, 1) / ...
                 AllSubjects_Data(Idx).size.data(iSubj, 2), ...
        ...
        for iSubj = 1:NbSub
            plot((iROI - 1) * 1 + Scatter(iSubj), AllSubjects_Data(Idx2).size.data(iSubj, 1) / ...
                 AllSubjects_Data(Idx2).size.data(iSubj, 2), ...
                 'marker', '.', 'markersize', 30, ....
                 'color', COLOR_Subject(iSubj, :));
        end
        XtickLabel2{iROI} = strrep(AllSubjects_Data(Idx2).name, '_', ' ');
