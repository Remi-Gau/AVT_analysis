...
for iSub = 1:NbSub
    ...
    load(fullfile(Sub_dir, 'results', 'profiles', 'surf', 'correlations', ...
                  [SubLs(iSub).name '-SurfCorrelation.mat']), ...
    ...
    for iToPlot = 1:numel(ToPlot)
        ...
        for iCdt = 1:size(Cdt, 1)
            for iROI = 1:5
                CompilGrpBetaCrossSens(iSub, iToPlot, iCdt, iROI) = mean(Slope_CrossSens_fStim{iToPlot, ...
                                                                                               iCdt, ...
                                                                                               iROI});
                CompilGrpBetaSenseVSSense(iSub, iToPlot, iCdt, iROI) = mean(Slope_CrossSens_fCrossSens{iToPlot, ...
                                                                                                       iCdt, ...
                                                                                                       iROI});
            end
        end
        ...
        for iCdt = 1:size(Cdt, 1)
            for iROI = 1:5
                CompilGrpBetaSideVSSide(iSub, iToPlot, iCdt, iROI) = mean(Slope_CrossSide_fCrossSide{iToPlot, ...
                                                                                                     iCdt, ...
                                                                                                     iROI});
            end
        end
...
for iROI = 1:5
    ...
    for iCdt = 1:size(Cdt, 1)
        ...
        h = errorbar(Xpos, mean(tmp), nanstd(tmp), 'o', ...
                     'LineStyle', 'none', ...
                     'Color', [0 0 0]);
        set(h, ...
            'MarkerSize', 3, ...
