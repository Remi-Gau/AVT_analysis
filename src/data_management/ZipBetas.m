...
for iSub = 1:5 % 6:NbSub % for each subject
    ...
    for iBeta = 1:size(BetaOfInterest, 1)
        ...
        gzip(fullfile(StartDir, SubLs(iSub).name, dest_dir, 'betas', ['r' SubLs(iSub).name '_' prefix 'beta-' sprintf('%04.0f', BetaOfInterest(iBeta)) '.nii']));
        if IsTarget
            movefile(fullfile(StartDir, SubLs(iSub).name, ffx_dir, 'betas', ['r' SubLs(iSub).name '_' prefix 'beta-' sprintf('%04.0f', BetaOfInterest(iBeta)) '.nii.gz']), ...
                     fullfile(StartDir, SubLs(iSub).name, dest_dir, 'betas', ['r' SubLs(iSub).name '_target_' prefix 'beta-' sprintf('%04.0f', BetaOfInterest(iBeta)) '.nii.gz']));
        end
    end
