...
function ExtractVolBeta
    ...
    Mask_Ori.ROI(1) = struct('name', 'V1_L_thres', ...
                             'fname', 'SubjName_lcr_V1_Pmap_Ret_thres_10_data.nii');
    Mask_Ori.ROI(end + 1) = struct('name', 'V2_L_thres', ...
                                   'fname', 'SubjName_lcr_V2_Pmap_Ret_thres_10_data.nii');
    Mask_Ori.ROI(end + 1) = struct('name', 'V3_L_thres', ...
                                   'fname', 'SubjName_lcr_V3_Pmap_Ret_thres_10_data.nii');
    Mask_Ori.ROI(end + 1) = struct('name', 'V4_L_thres', ...
                                   'fname', 'SubjName_lcr_V4_Pmap_Ret_thres_10_data.nii');
    Mask_Ori.ROI(end + 1) = struct('name', 'V5_L_thres', ...
                                   'fname', 'SubjName_lcr_V5_Pmap_Ret_thres_10_data.nii');

    Mask_Ori.ROI(end + 1) = struct('name', 'A1_L', ...
                                   'fname', 'SubjName_A1_lcr_RG_data.nii');

    Mask_Ori.ROI(end + 1) = struct('name', 'PT_L', ...
                                   'fname', 'rwA41-42_L.nii');

    Mask_Ori.ROI(end + 1) = struct('name', 'V1_R_thres', ...
                                   'fname', 'SubjName_rcr_V1_Pmap_Ret_thres_10_data.nii');
    Mask_Ori.ROI(end + 1) = struct('name', 'V2_R_thres', ...
                                   'fname', 'SubjName_rcr_V2_Pmap_Ret_thres_10_data.nii');
    Mask_Ori.ROI(end + 1) = struct('name', 'V3_R_thres', ...
                                   'fname', 'SubjName_rcr_V3_Pmap_Ret_thres_10_data.nii');
    Mask_Ori.ROI(end + 1) = struct('name', 'V4_R_thres', ...
                                   'fname', 'SubjName_rcr_V4_Pmap_Ret_thres_10_data.nii');
    Mask_Ori.ROI(end + 1) = struct('name', 'V5_R_thres', ...
                                   'fname', 'SubjName_rcr_V5_Pmap_Ret_thres_10_data.nii');

    Mask_Ori.ROI(end + 1) = struct('name', 'A1_R', ...
                                   'fname', 'SubjName_A1_rcr_RG_data.nii');

    Mask_Ori.ROI(end + 1) = struct('name', 'PT_R', ...
                                   'fname', 'rwA41-42_R.nii');
    ...
    for iSub = 1:NbSub
        ...
        for iROI = 1:length(Mask.ROI)
            Mask.ROI(iROI).fname = strrep(Mask_Ori.ROI(iROI).fname, ...
                                          'SubjName', ...
                                          SubLs(iSub).name);
        end
        ...
        MaskFile = fullfile(SubDir, ...
                            'ffx_nat', ...
                            'betas', ...
                            ['r' SubLs(iSub).name '_GLM_mask.nii']);
        ...
        if ~exist(MaskFile, 'file')
            ...
            try
                ...
                error('The GLM mask file %s is missing.', MaskFile);

            end

        end
        Mask.global.hdr = spm_vol(MaskFile);
        ...
        for i = 1:length(Mask.ROI)
            ...
            FeatureFile = fullfile(AnalysisFolder, ['Features_', ...
                                                    Mask.ROI(i).name, ...
                                                    '_l-', num2str(NbLayers), ...
                                                    '_s-', num2str(FWHM), '.mat']);
        ...
        for iROI = 1:numel(ROIs)
            ...
            % ROI index
            ROI_L = ismember(strrep( ...
                                    {Mask.ROI(1:numel(Mask.ROI) / 2).name}, ...
                                    '_L', ...
                                    ''), ...
                             ROIs{iROI});
            ...
            ROI_R = ismember( ...
                             strrep({Mask.ROI(numel(Mask.ROI) / 2 + 1:end).name}, ...
                                    '_R', ...
                                    ''), ...
                             ROIs{iROI});
