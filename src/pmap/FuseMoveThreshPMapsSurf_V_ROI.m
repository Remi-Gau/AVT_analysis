...
for iSub = 1:NbSub
    ...
    for iROI = 1:NbROI
        %% unzip files
        cd(fullfile(SubDir, 'pmap', ROI_dir(iROI).name, [ROI_dir(iROI).name '-A'], ...
                    'MeshDataToVolume'));
        gunzip('*data.nii.gz');
        movefile(fullfile(SubDir, 'pmap', ROI_dir(iROI).name, [ROI_dir(iROI).name '-A'], ...
                          'MeshDataToVolume', ...
                          '*data.nii'), ...
                 fullfile(SubDir, 'pmap', ROI_dir(iROI).name));

        cd(fullfile(SubDir, 'pmap', ROI_dir(iROI).name, [ROI_dir(iROI).name '-B'], ...
                    'MeshDataToVolume'));
        gunzip('*data.nii.gz');
        movefile(fullfile(SubDir, 'pmap', ROI_dir(iROI).name, [ROI_dir(iROI).name '-B'], ...
                          'MeshDataToVolume', ...
                          '*data.nii'), ...
                 fullfile(SubDir, 'pmap', ROI_dir(iROI).name));
