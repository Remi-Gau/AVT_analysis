...
for iSub = NbSub % for each subject
    ...
    cd(fullfile(SubDir, 'ffx_nat_smooth'));
    ...
    %% Define batch
    matlabbatch = {};
    matlabbatch{1}.spm.stats.con.spmmat = {fullfile(SubDir, 'ffx_nat_smooth', 'SPM.mat')};
    ...
    %% Evaluate contrasts
    cd(fullfile(SubDir));
    save (strcat('T_Contrast_Smooth_', SubLs(iSub).name, '_matlabbatch.mat'), 'matlabbatch');
