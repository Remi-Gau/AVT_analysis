    ...
    for ihs = 1:numel(hs)
        ...
        for iCond = 1:numel(CondNames)
            for iLayer = 1:NbLayers
                ...
                VTK_file = dir(['GrpSurf_' CondNames{iCond} '_layer_' num2str(iLayer) ...
                                '_' hs(ihs) 'h' ...
                                suffix '.vtk']); %#ok<*PFBNS>
                disp(VTK_file.name);
        ...
        for iCond = 1:numel(CrossSide)
            ...
            CrossSidelLayers = ...
                AllLayers(:, :, :, CrossSide{iCond}(1)) - AllLayers(:, :, :, CrossSide{iCond}(2));

            if Smooth
                CrossSidelLayersNoSmooth = ...
                    AllLayersNoSmooth(:, :, :, CrossSide{iCond}(1)) - AllLayersNoSmooth(:, :, :, CrossSide{iCond}(2));
            ...
            for NbSub2Excl = 0:(NbSub - 5)
                ...
                write_vtk(fullfile(DataFolder, [upper(hs(ihs)) 'H'], 'CrossSide', ...
                                   [CrossSideName{iCond} '_' hs(ihs) 'h_lin_inf' suffix '.vtk']), ...
                          InfVertex, ...
                          InfFace, ...
                          Lin');
                write_vtk(fullfile(DataFolder, [upper(hs(ihs)) 'H'], 'CrossSide', ...
                                   [CrossSideName{iCond} '_' hs(ihs) 'h_cst_inf' suffix '.vtk']), ...
                          InfVertex, ...
                          InfFace, ...
                          Cst');
                %                 write_vtk(fullfile(DataFolder, [upper(hs(ihs)) 'H'], 'CrossSide', ...
                %                     [CrossSideName{iCond} '_' hs(ihs) 'h_quad_inf' suffix '.vtk']), ...
                %                       InfVertex, ...
                %                       nfFace, ...
                %                       Quad')
                %                 write_vtk(fullfile(DataFolder, [upper(hs(ihs)) 'H'], 'CrossSide',...
                %                     [CrossSideName{iCond} '_' hs(ihs) 'h_mean_inf' suffix '.vtk']), ...
                %                       InfVertex, ...
                %                       InfFace, ...
                %                       Mean');
                %                 write_vtk(fullfile(DataFolder, [upper(hs(ihs)) 'H'], 'CrossSide',...
                %                     [CrossSideName{iCond} '_' hs(ihs) 'h_T_map_inf' suffix '.vtk']), ...
                %                       InfVertex, ...
                %                       InfFace, ...
                %                       T_map');
        ...
        if 0
            ...
            for iCond = 1:numel(CrossSens)
                ...
                CrossSenslLayers = ...
                    AllLayers(:, :, :, CrossSens{iCond}(1)) - AllLayers(:, :, :, CrossSens{iCond}(2));
                ...
                for NbSub2Excl = 0:(NbSub - 5)
                    ...
                    %                 X = [];
                    %                 for iSubj=1:(NbSub-NbSub2Excl)
                    %                     X((1:NbLayers)+NbLayers*(iSubj-1),...
                    %                       (1:size(DesMat,2))+size(DesMat,2)*(iSubj-1)) = ...
                    %                       DesMat; %#ok<*SAGROW>
                    %                 end
                    %
                    %                 B = pinv(X)*Y;
                    ...
                    %
                    %                         parfor iPerm = 1:size(ToPermute,1)
                    %                             % Perms(iPerm,:) = max(abs(mean(tmp.*repmat(ToPermute(iPerm,:)',...
                    %                                                           1,...
                    %                                                           size(tmp,2))))); %#ok<*SAGROW>
                    %                             Perms(iPerm,:) = mean(tmp.*repmat(ToPermute(iPerm,:)',...
                    %                                                           1,...
                    %                                                           size(tmp,2))); %#ok<*SAGROW>
                    %                         end
                    %
                    ...
                    %                         SPM_final(VertOfInt) = SPM;
                    %
                    %                         write_vtk(fullfile(DataFolder, [HS 'H'], ...
                    %                                   'Baseline', ...
                    %                                   'mask',...
                    %                                   [CondName{iCond} '_' ...
                    %                                       hs 'h_' Suffix{i} '_mask' suffix '.vtk']), ...
                    %                                   Vertex, ...
                    %                                   Face, ...
                    %                                   SPM_final')
                    %
                    ...
                    %                                   'mask', ...
                    %                                   [CondName{iCond} '_' ...
                    %                                       hs 'h_' Suffix{i} '_inf_mask' suffix '.vtk']), ...
                    %                                   InfVertex, ...
                    %                                   InfFace, ...
                    %                                   SPM_final')
                    %
                    ...
                    %                                   'mask',...
                    %                                   [CondName{iCond} '_' ...
                    %                                       hs 'h_' Suffix{i} '_inf_mask_thres' suffix '.vtk']), ...
                    %                                   InfVertex, ...
                    %                                   InfFace, ...
                    %                                   SPM_final')
                    %
                    ...
                    %                                   'mask',...
                    %                                   [CondName{iCond} '_' ...
                    %                                       hs 'h_' Suffix{i} '_inf_mask_pmap' suffix '.vtk']), ...
                    %                                   InfVertex, ...
                    %                                   InfFace, ...
                    %                                   PMaps_final')
                    %
                    ...
                    %                     [CrossSensName{iCond} '_' hs(ihs) 'h_lin_inf' suffix '.vtk']), ...
                    %                       InfVertex, ...
                    %                       InfFace, ...
                    %                       Lin')
                    %                 write_vtk(fullfile(DataFolder, [upper(hs(ihs)) 'H'], 'CrossSens', ...
                    %                     [CrossSensName{iCond} '_' hs(ihs) 'h_cst_inf' suffix '.vtk']), ...
                    %                       InfVertex, ...
                    %                       InfFace, ...
                    %                       Cst')
                    %                 write_vtk(fullfile(DataFolder, [upper(hs(ihs)) 'H'], 'CrossSens', ...
                    %                     [CrossSensName{iCond} '_' hs(ihs) 'h_quad_inf' suffix '.vtk']), ...
                    %                       InfVertex, ...
                    %                       InfFace, ...
                    %                       Quad')

                    write_vtk(fullfile(DataFolder, [upper(hs(ihs)) 'H'], 'CrossSens', ...
                                       [CrossSensName{iCond} '_' hs(ihs) 'h_mean_inf' suffix '.vtk']), ...
                              InfVertex, ...
                              InfFace, ...
                              Mean');
                    write_vtk(fullfile(DataFolder, [upper(hs(ihs)) 'H'], 'CrossSens', ...
                                       [CrossSensName{iCond} '_' hs(ihs) 'h_T_map_inf' suffix '.vtk']), ...
                              InfVertex, ...
                              InfFace, ...
                              T_map');
                end
