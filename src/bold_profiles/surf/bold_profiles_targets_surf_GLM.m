    ...
    for hs = 1:2
        ...
        for iCdt = 1:size(Cond2Contrast, 1)
            ...
            for iBeta = 1:size(B, 1)
                Mapping = zeros(1, size(inf_vertex, 2));
                Mapping(VertexWithData) = B(iBeta, :);
                write_vtk(fullfile(Results_dir, 'side', [SubLs(iSub).name '_' HsSufix 'cr_Targets_' ...
        ...
        for iCdt = 1:size(Cond2Contrast, 1)
            ...
            for iBeta = 1:size(B, 1)
                Mapping = zeros(1, size(inf_vertex, 2));
                Mapping(VertexWithData) = B(iBeta, :);
                write_vtk(fullfile(Results_dir, 'cross_sens', [SubLs(iSub).name '_' HsSufix 'cr_Targets' ...
