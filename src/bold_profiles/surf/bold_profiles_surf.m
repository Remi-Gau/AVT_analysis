...
for iSub = 1:NbSub
    ...
    for iROI = 5:7 % numel(ROI)
        ...
        for iCdt = 1:numel(Cdt_ROI_lhs)
            ...
            Data_ROI.Ispi.WholeROI.MEAN(1:size(tmpL, 2), iCdt, 1) = squeeze(nanmean(nanmean(tmpL, 3), 1));
            Data_ROI.Ispi.WholeROI.MEDIAN(1:size(tmpL, 2), iCdt, 1) = squeeze(nanmean(nanmedian(tmpL, 3), 1));

            Data_ROI.Ispi.LayerMean(:, 1:size(tmpL, 2), iCdt, 1) = nanmean(tmpL, 3);
            Data_ROI.Ispi.LayerMedian(:, 1:size(tmpL, 2), iCdt, 1) = nanmedian(tmpL, 3);

            tmpR = shiftdim(FeaturesR(:, :, Contra_Beta2Sel_rhs), 1);

            Data_ROI.Ispi.WholeROI.MEAN(1:size(tmpR, 2), iCdt, 2) = squeeze(nanmean(nanmean(tmpR, 3), 1));
            Data_ROI.Ispi.WholeROI.MEDIAN(1:size(tmpR, 2), iCdt, 2) = squeeze(nanmean(nanmedian(tmpR, 3), 1));

            Data_ROI.Ispi.LayerMean(:, 1:size(tmpR, 2), iCdt, 2) = nanmean(tmpR, 3);
            Data_ROI.Ispi.LayerMedian(:, 1:size(tmpR, 2), iCdt, 2) = nanmedian(tmpR, 3);
        ...
        Data_ROI.Contra.WholeROI.MEAN = nan(Nb_sess, size(Cdt_ROI_lhs, 1), 2);
        Data_ROI.Contra.WholeROI.MEDIAN = nan(Nb_sess, size(Cdt_ROI_lhs, 1));

        Data_ROI.Contra.LayerMean = nan(NbLayers, Nb_sess, size(Cdt_ROI_lhs, 1), 2);
        Data_ROI.Contra.LayerMedian = nan(NbLayers, Nb_sess, size(Cdt_ROI_lhs, 1), 2);
        ...
        for iCdt = 1:numel(Cdt_ROI_lhs)
            ...
            Data_ROI.Contra.WholeROI.MEAN(1:size(tmpL, 2), iCdt, 1) = squeeze(nanmean(nanmean(tmpL, 3), 1));
            Data_ROI.Contra.WholeROI.MEDIAN(1:size(tmpL, 2), iCdt, 1) = squeeze(nanmean(nanmedian(tmpL, 3), 1));

            Data_ROI.Contra.LayerMean(:, 1:size(tmpL, 2), iCdt, 1) = nanmean(tmpL, 3);
            Data_ROI.Contra.LayerMedian(:, 1:size(tmpL, 2), iCdt, 1) = nanmedian(tmpL, 3);

            tmpR = shiftdim(FeaturesR(:, :, Contra_Beta2Sel_rhs), 1);

            Data_ROI.Contra.WholeROI.MEAN(1:size(tmpR, 2), iCdt, 2) = squeeze(nanmean(nanmean(tmpR, 3), 1));
            Data_ROI.Contra.WholeROI.MEDIAN(1:size(tmpR, 2), iCdt, 2) = squeeze(nanmean(nanmedian(tmpR, 3), 1));

            Data_ROI.Contra.LayerMean(:, 1:size(tmpR, 2), iCdt, 2) = nanmean(tmpR, 3);
            Data_ROI.Contra.LayerMedian(:, 1:size(tmpR, 2), iCdt, 2) = nanmedian(tmpR, 3);
        ...
        Data_ROI.Contra_VS_Ipsi.LayerMean = nan(NbLayers, Nb_sess, size(Cdt_ROI_lhs, 1), 2);
        Data_ROI.Contra_VS_Ipsi.LayerMedian = nan(NbLayers, Nb_sess, size(Cdt_ROI_lhs, 1), 2);

        Data_ROI.Contra_VS_Ipsi.WholeROI.MEAN = nan(Nb_sess, size(Cdt_ROI_lhs, 1), 2);
        Data_ROI.Contra_VS_Ipsi.WholeROI.MEDIAN = nan(Nb_sess, size(Cdt_ROI_lhs, 1), 2);
        ...
        for iCdt = 1:size(Cdt_ROI_lhs, 1)
            ...
            for i = 1:2
                ...
                if ~all(isnan(tmp(:)))
                    Data_ROI.Contra_VS_Ipsi.WholeROI.MEAN(1:size(tmp, 2), iCdt, i) = squeeze(nanmean(nanmean(tmp, 3), 1));
                    Data_ROI.Contra_VS_Ipsi.WholeROI.MEDIAN(1:size(tmp, 2), iCdt, i) = squeeze(nanmean(nanmedian(tmp, 3), 1));

                    Data_ROI.Contra_VS_Ipsi.LayerMean(:, 1:size(tmp, 2), iCdt, i) = nanmean(tmp, 3);
                    Data_ROI.Contra_VS_Ipsi.LayerMedian(:, 1:size(tmp, 2), iCdt, i) = nanmedian(tmp, 3);
                end
            end
        ...
        Data_ROI.ContSensModIpsi.LayerMean = nan(NbLayers, Nb_sess, size(Cdt_ROI_lhs, 1), 2);
        Data_ROI.ContSensModIpsi.LayerMedian = nan(NbLayers, Nb_sess, size(Cdt_ROI_lhs, 1), 2);

        Data_ROI.ContSensModIpsi.WholeROI.MEAN = nan(Nb_sess, size(Cdt_ROI_lhs, 1), 2);
        Data_ROI.ContSensModIpsi.WholeROI.MEDIAN = nan(Nb_sess, size(Cdt_ROI_lhs, 1), 2);
        ...
        for iCdt = 1:size(Cdt_ROI_lhs, 1)
            ...
            for i = 1:2
                if i == 1
                    tmp = SensMod1_L - SensMod2_L;
                else
                    tmp = SensMod1_R - SensMod2_R;
                end
                ...
                if ~all(isnan(tmp(:)))
                    Data_ROI.ContSensModIpsi.WholeROI.MEAN(1:size(tmp, 2), iCdt, i) = squeeze(nanmean(nanmean(tmp, 3), 1));
                    Data_ROI.ContSensModIpsi.WholeROI.MEDIAN(1:size(tmp, 2), iCdt, i) = squeeze(nanmean(nanmedian(tmp, 3), 1));

                    Data_ROI.ContSensModIpsi.LayerMean(:, 1:size(tmp, 2), iCdt, i) = nanmean(tmp, 3);
                    Data_ROI.ContSensModIpsi.LayerMedian(:, 1:size(tmp, 2), iCdt, i) = nanmedian(tmp, 3);
                end
            end
        ...
        Data_ROI.ContSensModContra.LayerMean = nan(NbLayers, Nb_sess, size(Cdt_ROI_lhs, 1), 2);
        Data_ROI.ContSensModContra.LayerMedian = nan(NbLayers, Nb_sess, size(Cdt_ROI_lhs, 1), 2);

        Data_ROI.ContSensModContra.WholeROI.MEAN = nan(Nb_sess, size(Cdt_ROI_lhs, 1), 2);
        Data_ROI.ContSensModContra.WholeROI.MEDIAN = nan(Nb_sess, size(Cdt_ROI_lhs, 1), 2);
        ...
        for iCdt = 1:size(Cdt_ROI_lhs, 1)
            ...
            for i = 1:2
                ...
                if ~all(isnan(tmp(:)))
                    Data_ROI.ContSensModContra.WholeROI.MEAN(1:size(tmp, 2), iCdt, i) = squeeze(nanmean(nanmean(tmp, 3), 1));
                    Data_ROI.ContSensModContra.WholeROI.MEDIAN(1:size(tmp, 2), iCdt, i) = squeeze(nanmean(nanmedian(tmp, 3), 1));

                    Data_ROI.ContSensModContra.LayerMean(:, 1:size(tmp, 2), iCdt, i) = nanmean(tmp, 3);
                    Data_ROI.ContSensModContra.LayerMedian(:, 1:size(tmp, 2), iCdt, i) = nanmedian(tmp, 3);
                end
            end
        end
        ...
        save(fullfile(Results_dir, strcat('Data_Surf_', ROI(iROI).name, ...
