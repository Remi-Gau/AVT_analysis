...
function bold_profiles_targets_vol_grp_avg
    clc;
    clear;
    ...
    for iSub = 1:NbSub
        ...
        for iROI = 1:numel(ROIs)
            ...
            File2Load = fullfile(SaveDir, strcat('Data_targets_', AllSubjects_Data(iROI).name, '_l-', ...
    ...
    for iROI = 1:numel(AllSubjects_Data)
        ...
        save(fullfile(ResultsDir, strcat('Results_', AllSubjects_Data(iROI).name, ...
                                         '_VolTargetsQuadGLM_l-', num2str(NbLayers), '.mat')), 'Results');
    end

    save(fullfile(ResultsDir, strcat('ResultsVolTargetsQuadGLM_l-', num2str(NbLayers), '.mat')));
