...
for iSub = 8:NbSub
    ...
    for hs = 1:2
        ...
        write_vtk(fullfile(SubDir, 'roi', 'surf', [SubLs(iSub).name '_A1_' suffix 'cr_RG_UN_inf.vtk']), vertex, face, mapping);
        write_vtk(fullfile(SubDir, 'roi', 'surf', [SubLs(iSub).name '_A1_' suffix 'cr_RG_UN.vtk']), norm_vertex, norm_face, mapping);
