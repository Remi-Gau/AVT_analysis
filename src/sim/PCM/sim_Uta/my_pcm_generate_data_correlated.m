...
subplot(1, 4, 3);
imagesc(U * U');
colorbar; % approx. = G_orig because UU' = MWW'M' = MM' if W iid

% Generate data
num_partitions = 20;
part_vec = kron(ones(num_conds, 1), [1:num_partitions]');
cond_vec = kron([1:num_conds]', ones(num_partitions, 1));
X = repmat(eye(num_partitions), num_conds, 1); % figure;imagesc(X);
Z = kron(eye(num_conds), ones(num_partitions, 1)); % figure;imagesc(Z);
...
sub = 1;
data{sub} = Y;
partVec{sub} = part_vec;
condVec{sub} = cond_vec;
