    ...
    for iROI = 1:numel(ROI)
        ...
        for iCdt = 1:6
            ...
            FileName = fullfile(FigureFolder, 'cdt', ...
                                ['GrpLvl_raster_AllCdt_' CondNames{iCdt} ...
                                 '_' ToPlot{iToPlot} ...
                                 '_' ROI(iROI).name '.gif']);
            ...
            for n = 1:nMax
                ...
                h = figure('name', [strrep(ROI(iROI).name, ' ', '_') '-' ToPlot{iToPlot}], ...
                           'Position', FigDim, ...
                           'Color', [1 1 1], ...
                           'Visible', Visibility); %#ok<*UNRCH>
                ...
                for iCdt2 = 1:6
                    ...
                    errorbar(1:6, mean(MeanProfiles(Splits(n):Splits(n + 1), :)), ...
                             nansem(squeeze(mean(Profiles(Splits(n):Splits(n + 1), :, :))), 2), 'k', ...
                             'linewidth', 2);
                    for iSubj = 1:size(Profiles, 3)
                        plot(1:6, mean(Profiles(Splits(n):Splits(n + 1), :, iSubj)), ':k', ...
                             'linewidth', .5);
                    end
                    plot([1 6], [0 0], '--k');
                ...
                mtit([strrep(ROI(iROI).name, '_', ' ') ' - Percentile ' CondNames{iCdt} ' - ' ToPlot{iToPlot}], ...
                     'fontsize', 14, ...
                     'xoff', 0, ...
                     'yoff', .025);
