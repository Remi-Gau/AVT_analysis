...
for iSub = 1:NbSub
    ...
    if DoCrossSensfCrossSens
        All_X_sort = All_X_sort_CrossSens_fCrossSens;
        All_Profiles = All_Profiles_CrossSens_fCrossSens;
    end
            ...
            for iROI = 1:size(All_Profiles,3)
                ...
                for ihs = 1:size(All_Profiles,4)
                    ...
                    All_Subj_All_Profiles{iToPlot, iCdt, iROI, ihs}(:,:,iSub) =...
                        mean(All_Profiles{iToPlot, Idx, iROI, ihs},3); %#ok<*SAGROW>
...
for iToPlot = 1:size(All_Subj_All_Profiles,1)
    ...
    for iCdt = 2:2:size(All_Subj_All_Profiles,2)
        ...
        for iROI = 1:size(All_Subj_All_Profiles,3)
            ...
            for ihs = 1:size(All_Subj_All_Profiles,4)
                ...
                FigName = ['Raster_all_subjects_' ToPlot{iToPlot} '_' ROI(iROI).name '_' HS 'HS_'...
                    CrossCondName{Cdt2Plot(iCdt,1)} '--f(' CrossCondName{Cdt2Plot(iCdt,2)} ...
                    ')-NbBin=' num2str(NbBin) ] ;
                FileName = fullfile(FigureFolder,'crosssens', ...
                    [FigName '.tiff']);
