...
for NbLayers=6
    for WithQuad= 1
        for WithPerm = 0
            ...
            if WithQuad
                load(fullfile(ResultsDir, strcat('ResultsVolStimsTargetsPoolQuadGLM_l-', num2str(NbLayers), '.mat')), 'AllSubjects_Data') %#ok<*UNRCH>
            else
                load(fullfile(ResultsDir, strcat('ResultsVolStimsTargetsPoolNoQuadGLM_l-', num2str(NbLayers), '.mat')), 'AllSubjects_Data') %#ok<*UNRCH>
            end
