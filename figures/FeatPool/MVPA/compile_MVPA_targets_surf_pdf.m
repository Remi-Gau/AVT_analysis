...
for Norm = 6
    ...
    for iROI=1:numel(ROIs)
        ...
        cd(MVPAFigDir)
        A = dir([strrep(ROIs{iROI}, '_', '-')  '-Targets-Contra-vs-Ipsi-' SaveSufix(9:end-4)  suffix '.pdf']);
        B = dir([strrep(ROIs{iROI}, '_', '-')  '-Targets-Contra-vs-Ipsi-' SaveSufix(9:end-4) '_6Layers' suffix '.pdf']);
        C = dir([strrep(ROIs{iROI}, '_', '-') '-Targets-Between-Senses-Ipsi-' SaveSufix(9:end-4)  suffix '.pdf']);
        D = dir([strrep(ROIs{iROI}, '_', '-') '-Targets-Between-Senses-Ipsi-' SaveSufix(9:end-4) '_6Layers' suffix '.pdf']);
        E = dir([strrep(ROIs{iROI}, '_', '-') '-Targets-Between-Senses-Contra-' SaveSufix(9:end-4)  suffix '.pdf']);
        F = dir([strrep(ROIs{iROI}, '_', '-') '-Targets-Between-Senses-Contra-' SaveSufix(9:end-4) '_6Layers' suffix '.pdf']);
        ...
        system([...
            ...
            [ROIs{iROI} '_AVT_MVPA_targets_' SaveSufix(2:end-4) suffix '_' date '.pdf']) ' ' Command])
