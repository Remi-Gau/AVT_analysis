function bold_profiles_targets_vol_pool_hs_grp_avg
clc; clear;
...
for iSub = 1:NbSub
    ...
    for iROI=1:numel(ROIs)
        ...
        File2Load = fullfile(SaveDir, strcat('Data_targets_Pooled_', AllSubjects_Data(iROI).name, '_l-', ...
...
for iROI = 1:numel(AllSubjects_Data)
    ...
    save( fullfile(ResultsDir, strcat('Results_', AllSubjects_Data(iROI).name, ...
        '_VolTargetsPoolQuadGLM_l-', num2str(NbLayers), '.mat')), 'Results')
end

save( fullfile(ResultsDir, strcat('ResultsVolTargetsPoolQuadGLM_l-', num2str(NbLayers), '.mat')) )
