...
for iSub = 1:NbSub
    ...
    for iROI = 1:numel(ROI)
        ...
        for iCdt = 1:numel(Cdt_ROI_lhs)
            ...
            tmp = cat(3,tmpL,tmpR); clear tmpL tmpR
        ...
        for iCdt = 1:size(Cdt_ROI_lhs,1)
            ...
            Contra = cat(3,Contra_L,Contra_R); clear Contra_L Contra_R
            
            Ipsi_L = shiftdim(FeaturesL(:,:,Ipsi_Beta2Sel_lhs),1);
            Ipsi_R = shiftdim(FeaturesR(:,:,Ipsi_Beta2Sel_rhs),1);
            Ipsi = cat(3,Ipsi_L,Ipsi_R); clear Ipsi_L Ipsi_R
        ...
        for iCdt = 1:size(Cdt_ROI_lhs,1)
            ...
            SensMod1 = cat(3,SensMod1_L,SensMod1_R); clear SensMod1_L SensMod1_R
            
            SensMod2_L = shiftdim(FeaturesL(:,:,SensMod2_Beta2Sel_lhs),1);
            SensMod2_R = shiftdim(FeaturesR(:,:,SensMod2_Beta2Sel_rhs),1);
            SensMod2 = cat(3,SensMod2_L,SensMod2_R); clear SensMod2_L SensMod2_R
        ...
        save(fullfile(Results_dir, strcat('Data_targets_Pooled_Surf_', ROI(iROI).name, ...
