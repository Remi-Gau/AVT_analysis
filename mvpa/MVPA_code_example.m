...
opt.permutation.test = 1;  % do label permutation at the subject level
opt.session.curve = 0; % learning curves on a subsample of all the sessions
opt.session.proptest = 0.2; % proportion of all sessions to keep as a test set: NOT PROPERLY IMPLEMENTED!!
opt.verbose = 0;
...
opt.rfe.threshold = 0.01; % percent to keep at each iteration
opt.rfe.nreps = 20; % number of iterations

% SVM C/nu parameters and default arguments
opt.svm.machine = 'C-SVC'; % switch to C-SVR if you want regressions but things might break down
...
% Label permutation at the subject level
...
% Maximum numbers of CVs (limit for the label permutation and learning
% curve)
